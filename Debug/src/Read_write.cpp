...
//Write if file is empty 
	if(!ListWords.empty()) { 
	std::cout << "[DEBUG] Write(): File not empty" << std::endl;
...
//Write if string is 1 word and file is empty
		if(Words.size() == 1) { 
			std::cout << "[DEBUG] Write(): File is empty : Get only 1 word" << std::endl;
			MyFileOut << Words[0] << " " << "\n"; 
...
//Write if string have more word and file is empty
			...
			std::cout << "[DEBUG] Write(): File is empty : Get many word" << std::endl;
